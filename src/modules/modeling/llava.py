...
class LlavaForConditionalGeneration(LlavaPreTrainedModel):
    ...
    def tie_weights(self):
        return self.language_model.tie_weights() if not getattr(self.config, "delay_init", False) else None
...
class LlavaForSeqToSeqGeneration(LlavaPreTrainedModel):
    def __init__(self, config: LlavaConfig):
        super().__init__(config)
        # self.vision_tower = AutoModel.from_config(config.vision_config)
        ...
        if getattr(config, "delay_init", False):
            ...
            self.language_model = T5ForConditionalGeneration(
                config.text_config, torch_dtype=config.torch_dtype
            )
        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1
        self.post_init()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if decoder_attention_mask is not None:
                shift_attention_mask = decoder_attention_mask[..., 1:]
                shift_logits = logits[..., :-1, :][shift_attention_mask.to(logits.device) != 0].contiguous()
                shift_labels = labels[..., 1:][shift_attention_mask.to(labels.device) != 0].contiguous()
            else:
                shift_logits = logits[..., :-1, :].contiguous()
