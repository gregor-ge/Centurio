...
class StableLmModel(StableLmPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # inspired by https://github.com/pkunlp-icler/FastV/blob/main/src/transformers/src/transformers/models/llama/modeling_llama.py#L730
            ### prune
            if self.config.do_prune:
                if idx == self.config.prune_layer:
                    if image_mask is not None:
                        bs, s, hd = hidden_states.shape
                        if self.config.prune_keep == 0:
                            keep_mask = ~image_mask

                        elif self.config.prune_mode == "attention":
                            last_layer_attention = layer_outputs[1]
                            # get average attention from last token
                            last_token_attention_avg = torch.mean(last_layer_attention, dim=1)[:, -1]
                            # get the attention in image token
                            last_token_attention_avg_last_tok_image = last_token_attention_avg[image_mask > 0].reshape(bs, -1)
                            topk = int(self.config.prune_keep*last_token_attention_avg_last_tok_image.size(1))
                            top_attention_rank = last_token_attention_avg_last_tok_image.topk(topk).values[:, -2:-1].expand_as(last_token_attention_avg)
                            max_prune = last_token_attention_avg_last_tok_image.size(1) - topk
                            prune_image_mask = (image_mask * (last_token_attention_avg <= top_attention_rank))
                            prune_image_mask = prune_image_mask * ((prune_image_mask.cumsum(-1) -1)<max_prune)
                            keep_mask = ~prune_image_mask


                        elif self.config.prune_mode == "pool":
                            num_imgs = image_mask.sum(dim=1).max()
                            num_imgs_keep = int(self.config.prune_keep*num_imgs)
                            h = int(math.sqrt(num_imgs))
                            assert h * h == num_imgs
                            pool = int(math.sqrt(num_imgs_keep))
                            dwn = nn.AdaptiveAvgPool2d(pool)
                            image_hidden_state = hidden_states[image_mask].reshape(bs, -1, hd)
                            image_hidden_state = image_hidden_state.permute(0, 2, 1).reshape(bs, -1, h, h)
                            image_hidden_state = dwn(image_hidden_state)
                            image_hidden_state = image_hidden_state.flatten(2).transpose(1, 2)

                            keep_image_mask = image_mask * ((image_mask.cumsum(-1) - 1) < (pool**2))
                            hidden_states[keep_image_mask] = image_hidden_state.reshape(-1, hd)
                            keep_mask = (~image_mask) + keep_image_mask

                        elif self.config.prune_mode == "pool-linear":
                            num_imgs = image_mask.sum(dim=1).max()
                            num_imgs_keep = int(self.config.prune_keep*num_imgs)
                            dwn = nn.AdaptiveAvgPool1d(num_imgs_keep)
                            image_hidden_state = hidden_states[image_mask].reshape(bs, -1, hd)
                            image_hidden_state = image_hidden_state.permute(0, 2, 1)
                            image_hidden_state = dwn(image_hidden_state)
                            image_hidden_state = image_hidden_state.permute(0, 2, 1)

                            keep_image_mask = image_mask * ((image_mask.cumsum(-1) - 1) < (num_imgs_keep))
                            hidden_states[keep_image_mask] = image_hidden_state.reshape(-1, hd)
                            keep_mask = (~image_mask) + keep_image_mask

                        keep_mask_mask_hidden = keep_mask.unsqueeze(-1).repeat(1, 1, hd)
                        hidden_states = hidden_states[keep_mask_mask_hidden].reshape(bs, -1, hd)
                        attention_mask = original_attention_mask[keep_mask].reshape(bs, -1)
                    ...
                    # we are at generation here so we have to adjust attention mask and position ids
                    elif past_key_values is not None:
                        # figure out how many tokens we cut
                        prev_layer_key_cache = past_key_values.key_cache[idx-1]
                        this_layer_key_cache = past_key_values.key_cache[idx]
                        pruned = prev_layer_key_cache.size(2) - this_layer_key_cache.size(2) - hidden_states.size(1)
                        #left padded mask so cutting pruned from the end is as good as removing the actual pruned idxs
                        attention_mask = original_attention_mask[:, :-pruned]
                        past_key_values_length = past_key_values_length - pruned
                        position_ids = position_ids - pruned
                        if self._attn_implementation == "flash_attention_2":
                            # 2d mask is passed through the layers
                            attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
                        # for output_attentions case used fallback to eager attention realization
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    position_ids,
                    past_key_values,
                    output_attentions or (self.config.do_prune and idx == self.config.prune_layer-1 and self.config.prune_mode == "attention"),
                )
            else:
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions or (self.config.do_prune and idx == self.config.prune_layer-1 and self.config.prune_mode == "attention"),
                    use_cache=use_cache,
                )
